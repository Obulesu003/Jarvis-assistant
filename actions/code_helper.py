...
def _take_screenshot() -> Path | None:
    try:
        ...
        logging.getLogger("Code").info(f"📸 Screenshot: {screenshot_path}")
        return screenshot_path
    except Exception as e:
        logging.getLogger("Code").warning(f"️ Screenshot failed: {e}")
        return None
...
def _screen_debug_action(description, file_path, player, speak=None) -> str:
    ...
    logging.getLogger("Code").info('📸 Capturing screen for debug...')
...
Be specific and actionable. If you see an error message, quote it exactly."""
        ...
        logging.getLogger("Code").debug('Screen analysis complete')
